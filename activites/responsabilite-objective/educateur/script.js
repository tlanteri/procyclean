...
async function open(user){if(code.length!==6){$("#error-message").textContent="Code de session incorrect.";showTop($("#error-panel"));return}const sessionRef=ref(database,`sessions/${code}`),snap=await get(sessionRef);if(!snap.exists()||snap.val().activity!=="responsabilite-objective"||snap.val().facilitatorId!==user.uid){$("#error-message").textContent="Cette session n’est pas accessible.";showTop($("#error-panel"));return}if(snap.val().groupVersion==="solo-group-v1"){const destination=new URL("../../groupe/educateur.html",location.href);destination.searchParams.set("activity","responsabilite-objective");destination.searchParams.set("session",code);location.replace(destination.href);return}showTop(null);render(snap.val());onValue(sessionRef,value=>value.exists()&&render(value.val()))}
